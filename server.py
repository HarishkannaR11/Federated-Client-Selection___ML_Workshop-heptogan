...
class Server(ClientSelection, Aggregation):
    ...
    def client_selection_smart(self, client_list, args):
        scores = []

        for client in client_list:
            label_diversity = len(set(client.train_labels))  # Diversity
            sample_count = len(client.train_labels)
            speed = client.minibatch_time or 1.0  # Avoid division by zero

            score = (sample_count * label_diversity) / speed
            scores.append(score)

        sorted_ids = sorted(range(len(scores)), key=lambda i: scores[i], reverse=True)
        return sorted_ids[:args["num_clients_per_round"]]
